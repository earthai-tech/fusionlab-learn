...
def _float_env(name: str, default: float) -> float:
    v = getenv_stripped(name)
    return float(v) if v is not None and v != "" else float(default)

# --- Train options (allow env overrides for sensitivity sweeps) ----------
EPOCHS = int(
    getenv_stripped("EPOCHS_OVERRIDE") or cfg.get("EPOCHS", 50)
)
...
PDE_MODE_CONFIG = (
    getenv_stripped("PDE_MODE_OVERRIDE")
    or cfg.get("PDE_MODE_CONFIG", "both")
)

# GEOPRIOR_USE_EFFECTIVE_H = cfg.get("GEOPRIOR_USE_EFFECTIVE_H", True)
GEOPRIOR_KAPPA_MODE = (
    getenv_stripped("KAPPA_MODE_OVERRIDE")
    or cfg.get("GEOPRIOR_KAPPA_MODE", "kb")
)
GEOPRIOR_HD_FACTOR       = float(
    getenv_stripped("HD_FACTOR_OVERRIDE")
    or cfg.get("GEOPRIOR_HD_FACTOR", 0.6)
)

LAMBDA_CONS   = _float_env("LAMBDA_CONS_OVERRIDE",
                           cfg.get("LAMBDA_CONS", 0.1))
LAMBDA_GW     = _float_env("LAMBDA_GW_OVERRIDE",
                           cfg.get("LAMBDA_GW", 0.01))
LAMBDA_PRIOR  = _float_env("LAMBDA_PRIOR_OVERRIDE",
                           cfg.get("LAMBDA_PRIOR", 0.01))
LAMBDA_SMOOTH = _float_env("LAMBDA_SMOOTH_OVERRIDE",
                           cfg.get("LAMBDA_SMOOTH", 0.01))
LAMBDA_MV     = _float_env("LAMBDA_MV_OVERRIDE",
                           cfg.get("LAMBDA_MV", 0.01))
