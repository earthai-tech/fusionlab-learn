...
def _parse_args(argv: Optional[list] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            ...
            "GeoPrior physics payload."
        )
    )
    p.add_argument(
        "--physics-npz",
        required=True,
        help="Path to <CITY>_phys_payload_run_val.npz",
    )
    p.add_argument(
        "--outdir",
        default=None,
        help="Output directory for CSVs and plots (default: same as NPZ).",
    )
    p.add_argument(
        "--city",
        default=None,
        help="City name tag for output files (e.g., 'nansha').",
    )
    p.add_argument(
        "--model-name",
        default="GeoPriorSubsNet",
        help="Model name tag for output files.",
    )
