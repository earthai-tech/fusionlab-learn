...
def build_future_sequences_npz(
    ...
    *,
    ...
    """
    Build GeoPriorSubsNet-ready NPZ inputs for *true future* forecasts.

    The function constructs exactly **one sequence per spatial group**
    (e.g. per (lon, lat) pixel) whose temporal structure is:

    - History window (for the encoder):
        [train_end_year - (time_steps - 1), ..., train_end_year]
    - Forecast horizon (to be predicted):
        [forecast_start_year, ..., forecast_start_year + H - 1]

    where H = forecast_horizon_years.

    This is **not** used for training; it is designed for Stage-3
    inference where:
      * you already trained the model on Stage-1 sequences, and
      * you optionally have driver scenarios for the future years
        (rainfall, GWL scenarios, urban load, etc.).

    Behaviour
    ---------
    - `dynamic_features` are built only on the **history** window
      (shape: (N, time_steps, n_dynamic)).
    - `future_features` are built on:
        * mode == "tft_like":
            history + horizon (length = time_steps + H)
        * otherwise ("pihal_like"):
            horizon only (length = H)
    - `coords` and `H_field` are defined **only over the horizon**
      (shape: (N, H, 3) and (N, H, 1) respectively).
    - Optional "+targets" (subsidence, GWL) are taken over the horizon.
      If you do not know true future subsidence/GWL, just leave those
      columns as NaN in `df_scaled`; the saved NPZ will contain NaNs but
      they are never used in standard Stage-3 inference.

    Time handling
    -------------
    - If `df_scaled[time_col]` is an integer/float year column, the
      year values are used directly.
    - If `df_scaled[time_col]` is a datetime-like column, we work with
      `df_scaled[time_col].dt.year`. This makes the function robust to
      year-only or full datetime stamps, as long as there is one record
      per (group, year) in the NATCOM setup.

    Parameters
    ----------
    df_scaled : DataFrame
        Stage-1 scaled dataframe (typically the `_03_scaled.csv`).
        It may already contain scenario rows for 2023–2025, etc.
    time_col : str
        Column expressing the *discrete* time index used in config
        (e.g. "year" or a datetime column).
    time_col_num : str
        Numeric time coordinate used for PINN coords (e.g.
        "year_numeric" or "coord_t").
    lon_col, lat_col : str
        Spatial coordinate columns.
    subs_col, gwl_col : str
        Columns of subsidence and groundwater level (used *only* to
        create optional target arrays for the horizon).
    h_field_col : str
        Column used as input "H_field" (e.g. effective thickness),
        assumed to be constant in time for a given group in NATCOM.
    static_features, dynamic_features, future_features : list[str]
        Feature lists (names must exist in `df_scaled` if they are to be
        used).
    group_id_cols : list[str]
        Columns that uniquely identify a spatial group (e.g. [lon, lat]
        or [lon, lat, city]).
    train_end_year : int
        Last year used in training (e.g. 2022).
    forecast_start_year : int
        First year of the future horizon (e.g. 2023 = train_end_year+1).
    forecast_horizon_years : int
        Number of years to forecast (H).
    time_steps : int
        Number of historical steps in the encoder window.
    mode : {"tft_like", "pihal_like"}
        Determines the shape of `future_features`.
    model_name : str
        Name of the model (for logging / provenance only).
    artifacts_dir : str
        Directory where the NPZ files will be written.
    prefix : str, default="future"
        Prefix for NPZ file names (e.g. "future_inputs.npz").
    verbose : int, default=1
        Verbosity level.
    logger : logging.Logger or None, default=None
        Optional logger for `vlog`.
    ...
    dict
        ...
        {
          "future_inputs_npz": "<path>",
          "future_targets_npz": "<path>",
        }

    Notes
    -----
    - The function assumes that for each group you want to forecast,
      `df_scaled` contains rows for **all required years**:
        [train_end_year - (time_steps - 1), ..., train_end_year,
         forecast_start_year, ..., forecast_start_year + H - 1].
      Groups with missing years are silently dropped (with an info
    ...
    # ------------------------------------------------------------------
    # 1) Determine the history + horizon year ranges
    # ------------------------------------------------------------------
    H = int(forecast_horizon_years)
    hist_start_year = int(train_end_year) - (int(time_steps) - 1)
    hist_years = np.arange(hist_start_year, int(train_end_year) + 1)
    fut_years = np.arange(int(forecast_start_year),
                          int(forecast_start_year) + H)
    all_years_needed = np.concatenate([hist_years, fut_years])

    # Helper to convert the time column to "year-like" integers
    t_series = df_scaled[time_col]
    if pd.api.types.is_datetime64_any_dtype(t_series):
        years_all = t_series.dt.year.to_numpy()
    else:
        years_all = t_series.to_numpy()

    mask = np.isin(years_all, all_years_needed)
    df_win = df_scaled.loc[mask].copy()

    if df_win.empty:
        raise ValueError(
            "build_future_sequences_npz: no rows in df_scaled for the "
            f"required window years={all_years_needed.tolist()}."
        )
    ...
    # ------------------------------------------------------------------
    # 2) Per-group construction of sequences
    # ------------------------------------------------------------------
    coords_list = []
    dyn_list = []
    fut_list = []
    ...
    for gid, g in df_win.groupby(group_id_cols):
        # local time/years for this group
        g_time = g[time_col]
        if pd.api.types.is_datetime64_any_dtype(g_time):
            g_years = g_time.dt.year.to_numpy()
        else:
            g_years = g_time.to_numpy()

        # sort by time first (for reproducibility)
        order = np.argsort(g_years)
        g = g.iloc[order].reset_index(drop=True)
        g_years = g_years[order]

        # helper: pick exactly one row per requested year
        def _rows_for_years(years_required: np.ndarray):
            rows = []
            for y in years_required:
                ...
                if not np.any(m):
                    return None
                sub = g.loc[m]
                # If a numeric time index is available, use it as tiebreaker
                if time_col_num in sub.columns:
                    sub = sub.sort_values(time_col_num)
                row = sub.iloc[-1]  # last row in that year
                rows.append(row)
            return rows

        hist_rows = _rows_for_years(hist_years)
        fut_rows = _rows_for_years(fut_years)

        if hist_rows is None or fut_rows is None:
            dropped_groups += 1
            continue

        # ---------------- dynamic_features (history only) ----------------
        if dyn_cols:
            dyn_seq = np.stack(
                [
                ...
                axis=0,  # (time_steps, n_dyn)
            )
        else:
            dyn_seq = np.zeros((len(hist_rows), 0), dtype=np.float32)

        # ---------------- future_features (history+future or future) -----
        if fut_cols:
            if mode == "tft_like":
        ...
        # ---------------- coords (horizon only) --------------------------
        t_vals = np.array(
            [row[time_col_num] for row in fut_rows],
            dtype=np.float32,
        )
        lon_vals = np.array(
        ...
        # ---------------- H_field (horizon only) -------------------------
        if h_field_col in g.columns:
            h_vals = np.array(
        ...
        # ---------------- static_features (one vector per group) ---------
        if static_cols:
        ...
        # ---------------- optional targets over horizon ------------------
        if subs_col in g.columns:
            subs_vals = np.array(
    ...
    if not coords_list:
        raise ValueError(
            ...
            "history + horizon window. Check that your df_scaled "
            "contains all required years for (lon, lat) groups."
        )

    if dropped_groups and verbose:
        vlog(
            f"[Future] Dropped {dropped_groups} group(s) due to "
            "incomplete history/horizon window.",
            verbose=verbose,
            level=1,
            logger=logger,
        )
    ...
    **kwargs
    ...
    if include_coords and model_inputs:
        df_part, df_coord_cols = _add_coords_to_df(
            ...
            verbose=verbose,
            _logger=_logger,
            force_future_from=kwargs.get("forecast_start_year"),
        )
        all_data_dfs.append(df_part)
...
def _add_coords_to_df(
    ...
    verbose: int = 0,
    _logger: Any = None,
    force_future_from: Optional[float] = None,   
    ...
    if force_future_from is not None:
        ...
        vlog(f"  Overriding coord_t with forecast years starting at "
...
def _add_ids_to_df(
    ...
    if ids_np_array is not None and ids_np_array.shape[0] == num_samples:
        ...
        vlog(f"  [WARN] `ids_data_array` sample size"
