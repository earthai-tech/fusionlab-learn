...
def compute_mv_prior(
    ...
    *,
    ...
    """Storage identity prior residual."""
    ...
    out = tf_log(Ss_safe) - (tf_log(mv) + tf_log(gw))

    vprint(verbose, "mv_prior: Ss_safe=", Ss_safe)
...
def equilibrium_compaction_si(
    *,
    ...
    return Ss_field * delta_h * H_field_si
    ...
    *,
    ...
    def step(prev: Tensor, elems: Tuple[Tensor, Tensor, Tensor]) -> Tensor:
        ...
        if method == "exact":
            ...
            nxt = prev + dt_i * (seq_i - prev) / (tau_i + tf_constant(SMALL, tau_i.dtype))
        return nxt
    ...
    # scan returns (H,B,1) -> (B,H,1)
    return tf_transpose(s_tm, [1, 0, 2])
...
def compute_consolidation_step_residual(
    *,
    ...
    if dt is None:
        ...
        dt = _broadcast_like(_ensure_3d(tf_cast(dt, tf_float32)), s_state_si[:, 1:, :])
    dt_sec = dt_to_seconds(dt, time_units=time_units)
