   ...
   # @tf_autograph.experimental.do_not_convert
    ...
    def train_step(self, data):
        ...
        with GradientTape(persistent=True) as tape:
            ...
            tR_tf = None
            if coords_norm:
                tR, _, _ = coord_ranges(self.scaling_kwargs)
                if tR is not None:
                    tR_tf = tf_constant(float(tR), tf_float32)
    ...
    def _scale_param_grads(self, grads, trainable_vars):
        ...
        for g, v in zip(grads, trainable_vars):
            if g is None:
                ...
                continue
            mult = 1.0
            if self._mv_var is not None and v is self._mv_var:
                mult *= float(self._mv_lr_mult)
            if self._kappa_var is not None and v is self._kappa_var:
                mult *= float(self._kappa_lr_mult)
            scaled_grads.append(g * tf_cast(mult, g.dtype))
