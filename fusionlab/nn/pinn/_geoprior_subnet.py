...
class GeoPriorSubsNet(BaseAttentive):
    ...
    def _init_coordinate_corrections(
        ...
        * All branches take an input of shape ``(B, H, 3)`` corresponding
          ...
          dimension.
        * The corrections are **added before positivity constraints**
          are applied to :math:`K`, :math:`S_s`, and :math:`\tau`.
        """
    ...
    def _build_pinn_components(self):
        ...
        * The raw physics fields ``K_field``, ``Ss_field`` and
        ...
        # These are filled in split_physics_predictions() and can be
        # inspected via get_last_physics_fields() or used by diagnostic
        # routines.
        self.K_field = None
        self.Ss_field = None
        self.tau_field = None
    ...
    def split_physics_predictions(
        ...
        r"""
        Split physics-related outputs and enforce positivity constraints.
        ...
        * ``"phys_mean_raw"``: raw physics logits that will be
        ...
        tuple of tf.Tensor
            ...
            * ``s_pred_mean`` : subsidence mean, shape ``(B, H, 1)``,
            * ``gwl_pred_mean`` : head mean, shape ``(B, H, 1)``,
            * ``K_field`` : positive conductivity field,
            * ``Ss_field`` : positive storage field,
            * ``tau_field`` : positive relaxation time field.
