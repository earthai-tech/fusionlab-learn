...
from .ui.ui_splash import LoadingSplash
...
from .services.xfer_view import latest_xfer_csv, latest_xfer_json
...
class GeoPriorForecaster(QMainWindow):
    ...
    def __init__(
        ...
        *,
        ...
        # NEW: dialog for model-level HPs (memory, scales, attention, etc.)
    ...
    def _start_run_timer(self) -> None:
        ...
        if hasattr(self, "run_timer"):
            self.run_timer.setVisible(True)
            self.run_timer.restart()
            self.run_timer.cancel_hibernate()  # ensure full opacity
    ...
    def _stop_run_timer(self) -> None:
        ...
        if hasattr(self, "run_timer"):
            self.run_timer.stop()
            self.run_timer.schedule_hibernate(timeout_ms=60_000)  
    ...
    def _build_tuner_space_from_ui(self) -> Dict[str, Any]:
        ...
        space.update(
            self.model_params_dialog.to_search_space_fragment()
        )
    ...
    def _on_model_params_config(self) -> None:
        ...
        (or defaults). When the user clicks OK, the values stay inside
        the dialog instance and are picked up by
        :meth:`_build_tuner_space_from_ui`.
        """
        space = self.geo_cfg.tuner_search_space or default_tuner_search_space()
        defaults = default_tuner_search_space()
        self.model_params_dialog.load_from_space(space, defaults)
        ...
        if self.model_params_dialog.exec_() == QDialog.Accepted:
            self.log_updated.emit(
                "Model-level tuning parameters updated."
            )
    ...
    def _on_open_dataset(self) -> None:
        ...
        if not getattr(self, "_feature_tip_shown", False):
            QMessageBox.information(
                ...
                (
                    "Dataset selected.\n\n"
                    "Please open the “Feature config…” dialog to verify "
                    "the dynamic features and H-field column before "
                    "running the PINN."
                ),
            )
