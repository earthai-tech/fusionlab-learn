    ...
    * GeoPrior scalar parameters
    ...
    def __init__(
        ...
        *,
        ...
        info = QLabel(
            "Configure learning-rate multipliers and scalar "
            "GeoPrior parameters. These values control how the "
            "PINN physics block is initialised and optimised."
        )
        info.setWordWrap(True)
        main.addWidget(info)

        # ------------------------------------------------------------------
        # Learning-rate multipliers group
        # ------------------------------------------------------------------
        ...
        # --- learning-rate multipliers (dimensionless) ----------------
        ...
        lr_grid.addWidget(lbl_kappa_lr, 1, 0)
        lr_grid.addWidget(self.kappa_lr_spin, 1, 1)
        ...
        # m_v : constrained vertical compressibility [Pa^-1]
        ...
        lbl_mv.setToolTip(
            ...
            "in effective stress; enters seq = mᵥ γw Δh H."
        )
        gp_grid.addWidget(lbl_mv, row, 0)
        gp_grid.addWidget(self.init_mv_spin, row, 1)
        row += 1

        # κ̄ : dimensionless leakage / closure factor
        ...
        gp_grid.addWidget(lbl_kappa, row, 0)
        gp_grid.addWidget(self.init_kappa_spin, row, 1)
        row += 1

        # γ_w : unit weight of water [N m^-3]
        ...
        # h_ref : reference head [m]
        lbl_href = QLabel("Reference head href [m]:")
        lbl_href.setToolTip(
            "Reference groundwater head defining drawdown Δh = href − h̄.\n"
            "Used in the prior for equilibrium settlement seq( h̄ )."
        )
        gp_grid.addWidget(lbl_href, row, 0)
        gp_grid.addWidget(self.h_ref_spin, row, 1)
        row += 1
        ...
        gp_grid.addWidget(lbl_hd, row, 0)
        gp_grid.addWidget(self.hd_factor_spin, row, 1)
    ...
    def edit_physics(
        ...
        """
        initial = {
            k: cfg.get(k, v) for k, v in PHYSICS_DEFAULTS.items()
        }
