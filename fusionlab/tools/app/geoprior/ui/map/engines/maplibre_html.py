...
def maplibre_html() -> str:
    lines: List[str] = [
        ...
        "      if (!isFinite(lat) || !isFinite(lon)) continue;",
        "",
        "      const ang = Number(d.angle);",
        "      const a = isFinite(ang) ? (ang - 90) : 0;",
        "",
        ...
        "    _gpLoaded = true;",
        "",
        "    const p0 = String(lastBasemap.p || 'osm').toLowerCase();",
        "    const s0 = String(lastBasemap.s || 'light').toLowerCase();",
        "",
        "    // Default style (OSM light vector): no async style flip.",
        "    if (p0 === 'osm' && s0 === 'light') {",
        "      _ensurePointLayer();",
        "      _ensureHotLayers();",
        "",
        ...
        "    }",
        "",
        "    // Non-default basemap: setBasemap() will flip to the",
        "    // empty style and reinstall overlays on 'style.load'.",
        "    window.__GeoPriorMap.__ready = false;",
