...
class XferMapController(QObject):
    ...
    def _load_points(
        ...
        *,
        ...
        # 1. Identify City Name
        city_key = "xfer.city_a" if lid == "A" else "xfer.city_b"
        city_name = str(self._s.get(city_key, "") or "")

        # 2. Determine EPSG with Priority:
        #    Priority A: Manual Specific Override (UI SpinBox)
        #    Priority B: City Meta Registry (Loaded JSON)
        #    Priority C: Global Default (UI SpinBox)
        ...
        if not manual_specific:
            # Only lookup meta if manual specific is not set
       ...
       # Fallback: use global CRS keys if map keys unset.
        ...
        out, ok, msg = ensure_lonlat(
            out,
            mode=cm,
            utm_epsg=target_epsg, # Pass the resolved EPSG here
            src_epsg=target_epsg, # And here (ensure_lonlat handles distinction)
        )
        if not ok:
    ...
    # TODO : UPDATE AS point when all implementations are well done. 
    # class XferMapController(QObject):
    #     def __init__(self, ...):
    #         # ...
    #         self._view_factory = ViewFactory(store)
    
    #     def _render_from_store(self):
    #         # ... load df ...
            
    #         # INSTEAD OF: 
    #         # pts = self._as_points(df)
    #         # self._v.set_layer(..., pts, ...)
    
    #         # USE:
    #         payload = self._view_factory.build_layer(df, lid)
            
    #         if payload.kind == "points":
    #             self._v.set_layer(lid, name, payload.data, payload.opts)
    #         elif payload.kind == "hexbin_source":
    #             # You might need to add a new method to MapView/MapApi
    #             if hasattr(self._v, "set_hexbin_layer"):
    #                  self._v.set_hexbin_layer(lid, payload.data, payload.opts)
    #             else:
    #                  # Fallback if map doesn't support hexbins yet
    #                  self._v.set_layer(lid, name, payload.data, payload.opts)
