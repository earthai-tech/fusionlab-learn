...
# ---------- NEW: try cache ------------------------------------
manifest_path = _locate_manifest (_debug_mode ='debug') 

assert manifest_path.exists(), "manisfer file – cannot test inference"
cache_dir = Path(manifest_path).parent / "sequence_cache"
seqg, tr_ds, va_ds = SequenceGenerator.from_cache(
    cfg      = cfg,
...
if seqg is None:                             # no valid cache → compute
    seqg = SequenceGenerator(cfg, print)
    tr_ds, val_ds = seqg.run(
